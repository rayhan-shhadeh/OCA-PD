...
export default async function AboutPage({
  ...
  return (
    <div>
          ...
          <div className="bg-gradient-to-br from-primary-50 to-secondary-50 rounded-2xl p-8 md:p-12 border border-primary-100">
            <div className="flex items-center gap-3 mb-6">
              ...
              <h2 className="text-2xl font-black text-primary-800">
                {locale === "ar" ? "عن الجمعية" : "About the Association"}
              </h2>
            </div>
        ...
        <div className="container-custom">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {t.about.objectives.map((obj: string, i: number) => (
              <div
                key={i}
                className="bg-gray-50 rounded-xl p-5 border border-gray-100 hover:border-primary-200 hover:bg-primary-50 transition-colors"
              >
          ...
          <div className="grid grid-cols-2 lg:grid-cols-3 gap-6">
            {t.about.achievements.map((item: any, i: number) => (
              <div key={i} className="glass rounded-xl p-6 text-center">
                <div className="text-4xl font-black text-secondary-300 mb-2">
                  {item.number}
                </div>
      ...
      {/* CTA */}
      <section className="py-16 bg-primary-50  border-primary-100">
        <div className="container-custom text-center">
          <h3 className="text-2xl font-black text-primary-800 mb-4">
          ...
          <div className="flex flex-wrap gap-4 justify-center">
            <Link
              href={`/${locale}/donate`}
              className="bg-primary-500 text-white px-8 py-3 rounded-full font-bold hover:bg-primary-600 transition-colors shadow-md"
            >
              {t.common.donate}
            </Link>
            <Link
              href={`/${locale}/volunteer`}
              className="border-2 border-primary-500 text-primary-600 px-8 py-3 rounded-full font-bold hover:bg-primary-50 transition-colors"
            >
              {t.common.volunteer}
            </Link>
          </div>
